...
const getQuestionSaveClassName = (state: QuestionSaveState): string => {
  if (state === "saving") {
...
const AssessmentFlow = ({ assessment }: { assessment: PublicAssessment }) => {
  ...
  return (
    <div className="space-y-5">
      <section className="rounded-2xl border border-cyan-200/80 bg-gradient-to-br from-[#0f4f8a] via-[#0f5da0] to-[#1b78c2] p-4 text-white shadow-[0_10px_26px_rgba(15,93,160,0.18)] sm:p-5">
        ...
        <p className="mt-2 text-sm text-cyan-50/95">{assessment.description}</p>
        <p className="mt-3 text-xs text-cyan-100/90">{assessment.simulationNotice}</p>
      </section>

      {step === "intro" && (
        <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_6px_18px_rgba(15,23,42,0.07)] sm:p-5">
          <h3 className="text-lg font-semibold text-slate-900">1. Antes de empezar</h3>
          ...
          <label className="mt-5 block text-sm font-medium text-slate-700" htmlFor="candidateId">
          ...
          <div className="mt-5 flex flex-col gap-3 sm:flex-row">
            ...
            {hasDraft && (
              <button
                ...
                className="inline-flex min-h-11 items-center justify-center rounded-xl border border-slate-300 px-5 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50"
              >
      ...
      {step === "questions" && currentSection && (
        <section className="space-y-3.5 rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_6px_18px_rgba(15,23,42,0.07)] sm:p-5">
          ...
          <div className="pt-1">
            ...
            <p className="mt-1 text-sm text-slate-600">{currentSection.section.description}</p>
          </div>
          ...
          <div className="space-y-4">
            {currentSectionQuestions.map((question, index) => {
              ...
              return (
                <article
                  key={question.id}
                  className="rounded-xl border border-slate-200 bg-slate-50/40 p-3.5 shadow-[0_3px_10px_rgba(15,23,42,0.04)]"
                >
          ...
          <div className="hidden sm:flex sm:flex-row sm:justify-between">
            <button
              ...
              className="inline-flex min-h-11 items-center justify-center rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 disabled:opacity-40"
            >
              Bloque anterior
            </button>
            ...
            {currentSectionIndex < sectionGroups.length - 1 ? (
              ...
              <button
                ...
                className="inline-flex min-h-11 items-center justify-center rounded-xl bg-emerald-700 px-4 py-2 text-sm font-semibold text-white"
              >
                Revisar y finalizar
              </button>
            )}
          ...
          <div
            className="sticky bottom-0 z-10 mt-3 rounded-xl border border-slate-200 bg-white/95 p-2.5 shadow-[0_-4px_14px_rgba(15,93,160,0.1)] sm:hidden"
          >
            <div className="mx-auto grid max-w-5xl grid-cols-2 gap-2">
              <button
                ...
                onClick={() => setCurrentSectionIndex((value) => Math.max(0, value - 1))}
                disabled={currentSectionIndex === 0}
                className="inline-flex min-h-11 items-center justify-center rounded-xl border border-slate-300 bg-white px-4 text-sm font-semibold text-slate-700 disabled:opacity-40"
              >
                Anterior
              </button>
              ...
              {currentSectionIndex < sectionGroups.length - 1 ? (
                ...
                <button
                  type="button"
                  onClick={() => setStep("review")}
                  className="inline-flex min-h-11 items-center justify-center rounded-xl bg-emerald-700 px-4 text-sm font-semibold text-white"
                >
                  Revisar
                </button>
              )}
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_6px_18px_rgba(15,23,42,0.07)] sm:p-5">
          ...
          <ul className="mt-4 space-y-2 text-sm text-slate-700">
            {sectionGroups.map((entry) => {
              ...
              return (
                <li key={entry.section.id} className="rounded-lg bg-slate-50 px-3 py-2">
          ...
          <div className="mt-5 hidden flex-col gap-3 sm:flex sm:flex-row">
            <button
              ...
              className="inline-flex min-h-11 items-center justify-center rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700"
            >
              Volver a preguntas
            </button>
            <button
          ...
          >
            <div className="mx-auto grid max-w-5xl grid-cols-2 gap-2">
              <button
                type="button"
                onClick={() => setStep("questions")}
                className="inline-flex min-h-11 items-center justify-center rounded-xl border border-slate-300 px-4 text-sm font-semibold text-slate-700"
              >
                Volver
              </button>
              <button
