...
export default async function PreparacionDelfDalfPage({
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      <section className="rounded-2xl bg-[#395D9F] px-6 py-8 text-white">
      ...
      <section className="mt-6 grid gap-4 md:grid-cols-2" id="tarifas">
        <article className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-wide text-[#E50046]">Tarifa A1-B2</p>
          <p className="mt-1 text-2xl font-bold text-gray-900">Pack 4 clases de 55 min · 150€</p>
          <p className="mt-2 text-sm text-gray-700">
            Valido para conversacion o preparacion DELF en niveles A1, A2, B1 y B2.
          </p>
        </article>
        <article className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-wide text-[#E50046]">Tarifa C1-C2</p>
          <p className="mt-1 text-2xl font-bold text-gray-900">Pack 4 clases de 55 min · 200€</p>
          <p className="mt-2 text-sm text-gray-700">
      ...
      <section className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {orderedLevels.map((level) => {
          ...
          return (
            ...
            >
              <p className="text-xs font-semibold uppercase tracking-wide text-[#E50046]">Nivel {track.level}</p>
      ...
      <section className="mt-10 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-6 space-y-4">
          {resourceGroups.map((group) => (
            ...
            >
              <summary className="cursor-pointer text-sm font-semibold text-[#395D9F]">
                ...
                {group.resources.map((resource) => (
                  <li key={resource.url} className="text-sm text-gray-800">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="font-medium text-[#E50046] underline"
                    >
                      {resource.label}
                    </a>
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-3">
          {officialExamWindows.map((window) => (
            <article key={window.period} className="rounded-xl border border-blue-100 bg-blue-50/60 p-4">
              <h3 className="text-sm font-semibold text-[#395D9F]">{window.period}</h3>
